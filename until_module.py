...
class Sigloss(nn.Module):
    def __init__(self, temperature=1.0):
        ...
        self.temperature = temperature  # 固定温度参数
...
class dual_softmax_loss(nn.Module):
    def __init__(self, temp=1000):
        super().__init__()
        self.temp = temp

    def forward(self, sim_matrix):
        sim_matrix = sim_matrix * F.softmax(sim_matrix / self.temp, dim=0) * len(sim_matrix)
        logpt = F.log_softmax(sim_matrix, dim=-1)
        logpt = torch.diag(logpt)
        loss = -logpt
        return loss.mean()  # 添加.mean()以保持与CrossEn一致的输出格式
