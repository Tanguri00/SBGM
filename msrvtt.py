...
class MSRVTT_TrainDataLoader(Dataset):
    ...
    def _get_text(self, video_id, caption=None):
        k = 1
        choice_video_ids = [video_id]
        pairs_text = np.zeros((k, self.max_words), dtype=np.int32) #dtype=np.long
        pairs_mask = np.zeros((k, self.max_words), dtype=np.int32)
        pairs_segment = np.zeros((k, self.max_words), dtype=np.int32)
    ...
    def _get_rawvideo(self, choice_video_ids):
        video_mask = np.zeros((len(choice_video_ids), self.max_frames), dtype=np.int32)#dtype=np.long
        max_video_length = [0] * len(choice_video_ids)
